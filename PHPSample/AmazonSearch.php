...
/* Define constants */
define('WSDL_FILE',   'http://soap.amazon.co.jp/schemas3/AmazonWebServices.wsdl');
define('TAG',         '');
...
class AmazonSearch
{
  ...
  {
    $Params['locale'] = 'jp';
    //$Params = $this->array_convert_encoding($Params, 'UTF-8');
    //print_r($Params);
	...
	$Ret = null;
     ...
     return $Ret;
     //return $this->array_convert_encoding($Ret, mb_internal_encoding(), 'UTF-8');
   ...
   *	the given product category ('books-jp', by default). Return an
   *	array of item details success, null on error.
   */

  function DoKeywordSearch($Keyword, $Type = 'lite', $Category = 'books-jp', $Sort = '' , $Max = DEFAULT_MAX)
  {
  //print_r($Keyword);
    /* Form the parameters */
    //$Params = array('keyword' => htmlentities(mb_convert_encoding($Keyword,"EUC-JP")),
...
//print_r($Params);
  ...
  function DoBrowseNodeSearch($BrowseNode, $Type = 'lite', $Category = 'books-jp', $Sort = '' , $Max = DEFAULT_MAX)
  {
    /* Form the parameters */
    $Params = array('browse_node' => htmlentities($BrowseNode),
		    'mode'        => htmlentities($Category),
		    'sort'        => $Sort,
		    'tag'         => $this->AssociateID,
		    'devtag'      => $this->Token,
  ...
  function DoAuthorSearch($Author, $Type = 'lite', $Category = 'books-jp', $Max = DEFAULT_MAX)
  {
    /* Form the parameters */
   ...
   *	within the given product category ('books-jp', by default).
   *	Return an array of item details success, null on error.
   */

  function DoManufacturerSearch($Manufacturer, $Type = 'lite', $Category = 'books-jp', $Max = DEFAULT_MAX)
  {
   ...
   *	within the given product category ('books-jp', by default). Return
   *	an array of item details on success, null on error.
   */

  function DoSimilaritySearch($ASIN, $Type = 'lite', $Category = 'books-jp', $Max = DEFAULT_MAX)
  {
    /* Form the parameters */
  ...
  function DoPowerSearch($Power, $Type = 'lite', $Category = 'books-jp', $Max = DEFAULT_MAX)
  {
    /* Form the parameters */
  ...
  function DoSellerSearch($SellerID, $OfferStatus = 'open', $Type = 'lite',
                          $Category = 'books-jp', $Max = DEFAULT_MAX)
  {
    /* Form the parameters */
  ...
  function DoExchangeSearch($ExchangeID, $Type = 'lite', $Max = DEFAULT_MAX)
  {
   ...
   function array_convert_encoding($var, $to, $from = 'auto')
   {
     if (is_array($var)) {
       foreach ($var as $key=>$val) {
          $var[$key] = $this->array_convert_encoding($val, $to, $from);
       }
       return $var;
     } else {
